...
for index, row in stocks.iterrows():
    ...
    try:
        ...
        if (df['Adj Close'].iloc[-1] > df[sma_col].iloc[-1] 
            and df['Adj Close'].iloc[-2] < df[sma_col].iloc[-2]):
...
# get sp500 data
# define default so it always exists
...
try:
    ...
    data['sp_pct_change'] = 0  # fallback

# create the body of the email and send it
with open('email_template.html', 'r') as file:
    html_template = file.read()
template = Template(html_template)
html_output = template.render(data_52w_high=l_52w_high,
